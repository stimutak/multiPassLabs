...
/**
 * LogoHeader renders the site navigation along with a configurable logo
 * variant.  It preserves the real‑time clock and entity ticker from
 * the original header but replaces the textual placeholder with a
 * scalable SVG mark.  Pass a different `variant` to switch between
 * the monogram, glyph and wordmark logos.
 */
export function LogoHeader({ variant = "monogram" }: LogoHeaderProps) {
    ...
    <header className="fixed top-0 w-full z-40 bg-black border-b border-green-500/20">
      <nav className="container mx-auto px-4 h-12 flex items-center justify-between font-mono text-xs">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Logo variant={variant} className="h-6 w-auto text-green-400" />
          </Link>
        ...
        <div className="flex items-center gap-4">
          <span
            ...
            style={{ color: currentEntity?.color || "#ffffff" }}
          >
